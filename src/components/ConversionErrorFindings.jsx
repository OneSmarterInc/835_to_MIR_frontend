...
function severityBadgeStyle(value) {
  ...
  if (severity === "ERROR" || severity === "REFUSE" || severity === "FAILED" || severity === "FAIL") {
    return { ...base, background: "#FDE8E6", color: "#B42318", borderColor: "#F7C9C4" };
  }
  if (severity === "HOLD") {
    return { ...base, background: "#FFF3D6", color: "#A15C00", borderColor: "#F3D9A5" };
  }
  if (severity === "WARN" || severity === "WARNING") {
    return { ...base, background: "#FFF8CC", color: "#7A6100", borderColor: "#E8DB8E" };
  }
  if (severity === "INFO" || severity === "ACTIVE") {
    return { ...base, background: "#E8F1FF", color: "#245EA8", borderColor: "#C9DCF7" };
  }
  if (severity === "PASS" || severity === "OK" || severity === "SUCCESS") {
    return { ...base, background: "#E4F5EE", color: "#087A5A", borderColor: "#BFE5D8" };
  }
  return { ...base, background: "#EEF1F5", color: "#526174", borderColor: "#D7DEE7" };
}

function statusBadgeStyle(value) {
  const status = String(value || "").toUpperCase();
  if (status === "ERROR" || status === "PARTIAL") return severityBadgeStyle("ERROR");
  if (status === "PROCESSING" || status === "HOLD") return severityBadgeStyle("HOLD");
  if (status === "ARCHIVED" || status === "COMPLETED") return severityBadgeStyle("SUCCESS");
  return severityBadgeStyle("INFO");
}

export default function ConversionErrorFindings({ trackedFiles = [], showHeading = true }) {
  ...
  return (
    <section style={{ marginTop: showHeading ? "18px" : "10px" }}>
      {showHeading && (
      ...
      <div className="card" style={{ padding: 0, overflowX: "auto" }}>
        <table className="datatable" style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {errorFiles.length === 0 ? (
              <tr><td colSpan="6" style={{ padding: "24px", textAlign: "center", color: "var(--ink-3)" }}>No validation files with errors or findings.</td></tr>
            ) : errorFiles.map((file) => (
              <tr key={file.id}>
                <td style={{ fontWeight: 600 }}>{file.original_filename || file.stored_filename || "—"}</td>
                <td><span style={statusBadgeStyle(file.status)}>{String(file.status || "ERROR").toUpperCase()}</span></td>
                ...
                <td>
                  <button type="button" className="btn" onClick={() => setSelectedFileId(String(file.id))} style={{ whiteSpace: "nowrap" }}>
                    View findings
                  </button>
                </td>
              </tr>
        ...
        <div className="card" style={{ marginTop: "14px", padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
            <div>
              ...
              <h3 style={{ margin: "4px 0 0", fontSize: "16px" }}>{selectedFile.original_filename || selectedFile.stored_filename || "Validation file"}</h3>
            </div>
            <button type="button" className="btn" onClick={() => setSelectedFileId("")}>Close findings</button>
          </div>

          <div style={{ overflowX: "auto" }}>
            <table className="datatable" style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr><th>RULE</th><th>GATE</th><th>CHECKS / ERROR</th><th>SOURCE</th><th>SEVERITY</th></tr></thead>
              <tbody>
                {selectedFile._findings.length === 0 ? (
                  ...
                  <tr key={finding.key}>
                    ...
                    <td><span style={severityBadgeStyle(finding.severity)}>{finding.severity}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
