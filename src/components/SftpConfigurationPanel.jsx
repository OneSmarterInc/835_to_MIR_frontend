  ...
  return <div className={`sftp-pro ${compact ? "compact" : ""}`}>
    ...
    <main className="sftp-pro-main">
      {loading ? <div className="sftp-pro-loading">Loading SFTP configuration…</div> : <>
        ...
        {section === "DEFAULT" && form.setup_all_paths !== false && <div className="sftp-pro-paths">{PATHS.map(([key, label]) => <label key={key}><span>{label}</span><div><input value={(form.route_paths || {})[key] || ""} onChange={(e) => changePath(key, e.target.value)} placeholder={`/${key.toLowerCase().replace("_", "/")}`} /><button type="button" title={`Browse ${label} folder`} aria-label={`Browse ${label} folder`} onClick={() => browse((form.route_paths || {})[key], (value) => changePath(key, value))}><FolderIcon /></button></div></label>)}</div>}
        {section !== "DEFAULT" && form.use_default === false && <label className="sftp-pro-path"><span>{ROUTES[section].find((r) => r.key === direction)?.label} folder</span><div><input value={form.remote_folder || ""} onChange={(e) => change("remote_folder", e.target.value)} placeholder="/remote/folder" /><button type="button" title="Browse remote folder" aria-label="Browse remote folder" onClick={() => browse(form.remote_folder, (value) => change("remote_folder", value))}><FolderIcon /></button></div></label>}
