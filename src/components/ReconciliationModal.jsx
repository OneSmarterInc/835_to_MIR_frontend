    ...
    <section className="recon-popup" role="dialog" aria-modal="true" aria-labelledby="recon-popup-title">
      ...
      <div className="recon-popup-body">
        ...
        {data && <>
          ...
          <div className="recon-review-heading"><div><div className="recon-eyebrow">CLAIM REVIEW QUEUE</div><h3 className="recon-section-title">Needs a person</h3></div><span>{Number(data.review_pagination?.total || reviewRecords.length).toLocaleString()} items</span></div>
          <div className="recon-review-controls">
            <label className="recon-control search"><span aria-hidden="true">⌕</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search claim, amount, outcome, or action" aria-label="Search reconciliation review entries" /></label>
            <label className="recon-control filter"><span aria-hidden="true">▽</span><select value={outcomeFilter} onChange={(event) => setOutcomeFilter(event.target.value)} aria-label="Filter by outcome"><option value="">All outcomes</option>{outcomeOptions.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></label>
            <label className="recon-control filter"><span aria-hidden="true">◇</span><select value={actionFilter} onChange={(event) => setActionFilter(event.target.value)} aria-label="Filter by action"><option value="">All actions</option><option value="YET_TO_START">Yet to Start</option><option value="IN_PROCESS">In Process</option><option value="HOLD">Hold</option><option value="REJECTED">Rejected</option><option value="APPROVED">Approved</option></select></label>
          </div>
          <div className="recon-table-wrap"><table className="recon-review-table"><thead><tr><ReviewHeader label="Highmark Claim Number" field="highmark_claim_number" sort={sort} arrow={sortArrow} onSort={changeSort} /><ReviewHeader label="Internal Claim Number" field="internal_claim_number" sort={sort} arrow={sortArrow} onSort={changeSort} /><ReviewHeader label="Total Amount in MIR" field="mir901" sort={sort} arrow={sortArrow} onSort={changeSort} /><ReviewHeader label="Total Amount in RECON" field="recon_mir907" sort={sort} arrow={sortArrow} onSort={changeSort} /><ReviewHeader label="Outcome" field="status" sort={sort} arrow={sortArrow} onSort={changeSort} /><ReviewHeader label="Difference" field="difference" sort={sort} arrow={sortArrow} onSort={changeSort} /><ReviewHeader label="Action" field="action_status" sort={sort} arrow={sortArrow} onSort={changeSort} /></tr></thead>
            <tbody>{reviewRecords.length ? reviewRecords.map((row) => { const parts = claimParts(row); const action = row.action_status || "YET_TO_START"; return <tr key={row.claim_id}><td>{parts.highmarkClaimNumber || "—"}</td><td>{row.mir_claim_id && onOpenClaim ? <button type="button" className="recon-claim-link" onClick={() => onOpenClaim({ mir_claim_id: row.mir_claim_id })}>{parts.internalClaimNumber || "—"}</button> : (parts.internalClaimNumber || "—")}</td><td>{money(row.mir901)}</td><td>{money(row.recon_mir907)}</td><td><span className="recon-tag bad">{statusLabel(row.status)}</span></td><td>{money(row.difference)}</td><td><div className={`recon-action-field ${action.toLowerCase()}`}><span aria-hidden="true" /><select className="recon-action-select" value={action} onChange={(event) => updateAction(row.claim_id, event.target.value)} aria-label={`Action for claim ${row.claim_id}`}><option value="YET_TO_START">Yet to Start</option><option value="IN_PROCESS">In Process</option><option value="HOLD">Hold</option><option value="REJECTED">Rejected</option><option value="APPROVED">Approved</option></select></div></td></tr>; }) : <tr><td colSpan="7" className="recon-empty">No non-clear records require review.</td></tr>}</tbody>
          </table></div>
