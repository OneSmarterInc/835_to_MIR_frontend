...
export default function ResultView({ clients = [], isAdmin = false, initialClientId = "", selectedClient }) {
  ...
  return <section className="view on result-view table-screen">
    ...
    <div className="card result-table-card"><div className="result-table-wrap"><table className="reconciliation-table"><colgroup><col className="result-col-claim" /><col className="result-col-claim" /><col className="result-col-patient" /><col className="result-col-file" /><col className="result-col-file" /><col className="result-col-money" /><col className="result-col-money" /><col className="result-col-difference" /><col className="result-col-status" /></colgroup><thead><tr><SortableHeader label="Highmark Claim Number" sortKey="highmark_claim_number" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="Internal Claim Number" sortKey="internal_claim_number" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="Patient name" sortKey="patient_name" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="MIR file" sortKey="mir_filename" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="RECON file" sortKey="recon_filename" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="Amount in MIR" sortKey="amount_to_pay" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="Amount in RECON" sortKey="recon_paid_amount" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="Difference" sortKey="difference_amount" sort={sort} onSort={changeSort} arrow={sortArrow} /><SortableHeader label="Status" sortKey="status" sort={sort} onSort={changeSort} arrow={sortArrow} /></tr></thead><tbody>{rows.length ? rows.map((row) => { const parts = claimParts(row); return <tr key={row.mir_claim_id || `recon-${row.claim_id}`}><td><b>{parts.highmarkClaimNumber || "—"}</b></td><td><b>{parts.internalClaimNumber || "—"}</b></td><td>{row.mir_claim_id ? <button className="result-name-link" onClick={() => openClaim(row)}>{row.patient_name || "View claim"}</button> : <span>RECON-only claim</span>}<small>{row.member_id || "—"}</small></td><td>{row.mir_filename || "—"}</td><td><ReconMatches row={row} onShowMore={() => setMatchHistory(row)} /></td><td className="num">{money(row.amount_to_pay)}</td><td className="num">{money(row.recon_paid_amount)}</td><td className="num">{hasMirAndRecon(row) ? money(row.difference_amount) : "-"}</td><td><span className={`tag ${tone(row.status)}`}>{label(row.status)}</span></td></tr>; }) : <tr><td colSpan="9" className="result-empty">No MIR or RECON claims match this search and status filter.</td></tr>}</tbody></table></div><div className="result-pagination">
...
function HeldReviewModal({ review, onClose }) {
  ...
  return <div className="result-detail-backdrop" onClick={onClose}><div className="result-detail result-held-modal" role="dialog" aria-modal="true" aria-labelledby="held-review-title" onClick={(event) => event.stopPropagation()}><div className="result-detail-title"><div><div className="eyebrow">RECON processing review</div><h2 id="held-review-title">Held for Review</h2><p className="sub">{review.file?.original_filename} · {errors.length} {errors.length === 1 ? "record" : "records"} not imported</p></div><button className="btn" onClick={onClose}>Close</button></div><div className="result-table-wrap"><table className="result-held-table"><thead><tr><th>Row</th><th>Highmark Claim Number</th><th>Internal Claim Number</th><th>Reason</th><th>Source record</th></tr></thead><tbody>{errors.length ? errors.map((error, index) => { const parts = claimParts(error); return <tr key={`${error.row_number}-${error.error_code}-${index}`}><td>{error.row_number || "—"}</td><td><b>{parts.highmarkClaimNumber || "—"}</b></td><td><b>{parts.internalClaimNumber || "Not identified"}</b></td><td><span className="tag bad">{String(error.error_code || "REVIEW").replaceAll("_", " ")}</span><small>{error.error_message || "Record requires review."}</small></td><td><code>{error.raw_record || "—"}</code></td></tr>; }) : <tr><td colSpan="5" className="result-empty">No held-record details are available.</td></tr>}</tbody></table></div></div></div>;
}
