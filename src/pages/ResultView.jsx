...
export default function ResultView({ clients = [], isAdmin = false, initialClientId = "" }) {
  ...
  return <section className="view on result-view">
    ...
    <div className="filters-bar result-filters"><form className="result-global-search" onSubmit={runSearch}><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search claim ID, patient, member ID, or MIR file" aria-label="Search all results" /><button className="btn-gray" type="submit">Search</button>{activeSearch && <button className="btn-gray" type="button" onClick={() => { setSearch(""); setActiveSearch(""); setPage(1); loadResults(1, ""); }}>Clear</button>}</form><span className="runs-counter">{data.total_claims ?? rows.length} MIR claims · all processed RECON files</span></div>
