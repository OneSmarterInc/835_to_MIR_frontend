...
function NoticeCard({ notice, loadDetail, onReanalyze, onSelectClaim, onReview }) {
  ...
  return <article className="mpl-notice-card">
    ...
    {!!notice.extracted_claim_numbers?.length && <section className="mpl-extracted"><h3>Extracted Claims</h3><div className="mpl-claim-chips">{notice.extracted_claim_numbers.map((number) => <span key={number}>{number}</span>)}</div></section>}
    {notice.ai_response && <section className="mpl-ai-response"><span>AI RESPONSE</span><p>{notice.ai_response}</p>{!!notice.ai_suggestions?.length && <><h4>Suggested next steps</h4><ol>{notice.ai_suggestions.map((suggestion, index) => <li key={index}>{suggestion}</li>)}</ol></>}</section>}
