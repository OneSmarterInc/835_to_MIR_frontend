...
export default function SftpAutomationView({ clients = [], activeClientId = '', onSelectClient }) {
  ...
  return <section className="view on sftp-auto-view table-screen">
    ...
    <div className="card sftp-auto-table-wrap"><table className="datatable sftp-auto-runs">
      <colgroup><col className="sftp-auto-col-automation" /><col className="sftp-auto-col-direction" /><col className="sftp-auto-col-taken" /><col className="sftp-auto-col-sent" /><col className="sftp-auto-col-time" /><col className="sftp-auto-col-status" /></colgroup>
      <thead><tr><th>Automation</th><th>Direction</th><th>Files taken</th><th>Files sent</th><th>Scheduled / completed (Eastern)</th><th>Status</th></tr></thead>
      <tbody>{runs.length ? runs.map(run => <tr key={run.id}><td className="sftp-auto-run-type"><b>{run.automation_type}</b><small>{run.client_name}</small></td><td className="sftp-auto-cell-center"><span className="sftp-auto-direction">{run.direction || 'INCOMING'}</span></td><td className="sftp-auto-file-cell"><FileList files={run.input_files} empty="No files taken" /></td><td className="sftp-auto-file-cell"><FileList files={run.sent_files?.length ? run.sent_files : run.mir_output_files} empty="No files sent" /></td><td className="sftp-auto-run-time"><TimeDisplay value={run.scheduled_for} includeSeconds easternOnly /><small>{run.finished_at ? <>Completed: <TimeDisplay value={run.finished_at} includeSeconds easternOnly /></> : run.started_at ? 'In progress' : 'Waiting'}</small></td><td className="sftp-auto-run-status"><span className={`tag ${statusClass(run.status)}`}>{run.status}</span>{run.error_message && <small className="sftp-auto-error">{run.error_message}</small>}</td></tr>) : <tr><td colSpan="6" className="sftp-auto-none">No automation runs recorded for this client.</td></tr>}</tbody>
    </table></div>
