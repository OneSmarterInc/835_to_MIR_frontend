...
export default function ClaimSearchView({ clients, activeClientId, onSelectClient }) {
  ...
  return <section className="view on claim-search-view">
    ...
    <div className="claim-search-upload"><div><label>Upload and process 837 files</label><input id="search-837-upload" type="file" multiple accept=".837,.x12,.edi,.txt,.dat" onChange={event => setUploads(Array.from(event.target.files || []))} /><small>{uploads.length ? `${uploads.length} file(s) selected` : 'Select one or more 837 files.'}</small></div><button type="button" className="btn primary" disabled={!activeClientId || !uploads.length || processing} onClick={processUpload}>{processing ? 'Processing 837…' : 'Upload & Process 837'}</button></div>
